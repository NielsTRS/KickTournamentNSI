...
function keyPressed() {
    if (!player.death()) {
        if (keyCode === LEFT_ARROW || key === "q" && !menuOpened) player.left = true;
        if (keyCode === RIGHT_ARROW || key === "d" && !menuOpened) player.right = true;
...
function keyReleased() {
    if (keyCode === LEFT_ARROW || key === "q" && !menuOpened) player.left = false;
    if (keyCode === RIGHT_ARROW || key === "d" && !menuOpened) player.right = false;
}
