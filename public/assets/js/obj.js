...
class Player {
    ...
    update() {
        ...
        if (this.y === windowHeight - this.h - Floor / 2 && !this.lockSound) {
            this.jumps.count = 2;
            this.lockSound = true;
        }
        if (this.y !== windowHeight - this.h - Floor / 2) this.lockSound = false;
        this.y += this.vy;
    ...
    jump() {
        if (this.jumps.count > 0) {
            try {
                playSound('public/assets/sound/jump.wav', volume)
            } catch (err) {
            }
            this.jumps.count--;
            this.vy = -5;
    ...
    constrain() {
        this.x = constrain(this.x, 0, windowWidth - this.w);
        this.y = constrain(this.y, 0, windowHeight - this.h - Floor / 2);
    }
